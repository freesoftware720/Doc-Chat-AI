...
interface ChatHistoryClientPageProps {
    history: ChatSession[];
}

export function ChatHistoryClientPage({ history }: ChatHistoryClientPageProps) {
                    ...
                    <ul className="space-y-3">
                        {history.map(session => (
                            <li key={session.document_id}>
                                <div 
                                    className="flex items-center justify-between p-4 rounded-lg transition-colors hover:bg-muted/50 border cursor-pointer"
                                    onClick={() => handleChatClick(session.document_id)}
                                >
                                    ...
                                    <Button variant="outline" size="sm" asChild={false} onClick={(e) => { e.stopPropagation(); handleChatClick(session.document_id); }}>
                                        Continue Chat
