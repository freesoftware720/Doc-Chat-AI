...
const formSchema = z.object({
  ...
  landing_page_content: z.string().refine((val) => {
    try {
      JSON.parse(val);
      return true;
    } catch (e) {
...
export function AppSettingsForm({ settings }: { settings: AppSettings }) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      landing_page_content: JSON.stringify(settings.landing_page_content, null, 2),
    },
  });
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormDescription>The maximum number of messages a free user can send daily.</FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
            ...
            <AccordionItem value="landing-page">
                ...
                <AccordionContent className="pt-6">
                     <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormLabel>Landing Page JSON</FormLabel>
                                <FormControl>
                                    <Textarea
                                        className="font-mono text-xs min-h-[400px]"
                                        placeholder="Enter the landing page content as a JSON object."
                                        {...field}
                                    />
                                </FormControl>
                                <FormDescription>Edit the content for all sections of the public landing page. Be careful to maintain the correct JSON structure.</FormDescription>
                                <FormMessage />
                            </FormItem>
                        )}
                    />
