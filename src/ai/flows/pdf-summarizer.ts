 ...
 * - SummarizePdfInput - The input type for the summarizePdf function, which includes the PDF content as a data URI.
 * - SummarizePdfOutput - The return type for the summarizePdf function, which includes the summary of the PDF.
 */
...
const SummarizePdfInputSchema = z.object({
  pdfDataUri: z
    .string()
    .describe(
      'The PDF document content as a data URI that must include a MIME type and use Base64 encoding. Expected format: \'data:<mimetype>;base64,<encoded_data>\'.' // Explicitly define the expected format
    ),
});
...
const summarizePdfPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert summarizer. Please provide a concise summary of the following PDF document.  Make sure to include the key points and main ideas.
